...
export const getUsuario = async (req, res) => {
  ...
  export const postProductos=async(req,res)=>{
    try {
      ...
      const [result] = await pool.query("SLECT MAX(id) AS last_id FROM productos");

        //2.Obtenemos el último ID y lo incrementamos
        const lastId = result[0].last_id || 0; //Si no hay productos, el último ID sera 0
        const newId = lastId + 1; //Calculamos el nuevo ID

        //3. Insertamos el nuevo producto con el ID incrementado
        const [insertResult] = await pool.query(
            "INSERT INTO productos(id, nombre, description, precio_costo, precio_venta, cantidad,fotografia) VALUES (?,?,?,?,?,?,?)",
        ...
        if(insertResult.affectedRows > 0){
            ...
            res.statuus(404).json({ message:"No se ingresó el producto"});
        }
